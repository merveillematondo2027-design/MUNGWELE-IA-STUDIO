...
export async function notifyUser(userId:string,actor:{id:string;name:string},type:string,message:string,postId?:string){if(!userId||userId===actor.id)return;await addDoc(collection(db,'notifications'),{userId,actorId:actor.id,actorName:actor.name,type,message,postId:postId||null,read:false,createdAt:new Date().toISOString()}).catch(()=>undefined);}
...
export async function reportPost(post:CommunityPost, reporter:{id:string;name:string}, reason='Contenu signalé par un membre'){
  ...
  await addDoc(collection(db,'reports'),{userId:reporter.id,reporterId:reporter.id,reporterName:reporter.name||'Utilisateur',postId:post.id,generationId:post.generationId||post.id,ownerId:post.userId,ownerName:post.authorName,reason,status:'open',createdAt:new Date().toISOString()});
}
export async function sendMessage(sender:{id:string;name:string},recipientId:string,recipientName:string,text:string){const value=text.trim();if(!value||!recipientId||recipientId===sender.id)return;const members=[sender.id,recipientId].sort();const conversationId=members.join('_');const now=new Date().toISOString();await setDoc(doc(db,'conversations',conversationId),{userId:sender.id,members,memberNames:{[sender.id]:sender.name,[recipientId]:recipientName},lastMessage:value.slice(0,500),updatedAt:now},{merge:true});await addDoc(collection(db,'conversations',conversationId,'messages'),{userId:sender.id,senderId:sender.id,senderName:sender.name,recipientId,text:value.slice(0,2000),createdAt:now,read:false});}
