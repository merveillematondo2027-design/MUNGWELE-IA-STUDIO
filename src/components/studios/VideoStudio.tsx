...
export const VideoStudio: React.FC = () => {
  ...
  const chooseType = (next: VideoType) => {
    if (next === 'music_clip') {
      localStorage.removeItem(RESUME_PROJECT_KEY);
      sessionStorage.setItem(NEW_PROJECT_KEY, 'clips');
      setActiveStudio('clips');
      setActiveTab('studio-clips');
      addNotification('info', 'Clips Vidéo', 'MUNGWELE ouvre le studio spécialisé pour créer un clip musical.');
      ...
      <div className="flex flex-1 flex-col justify-end rounded-[28px] border border-white/10 bg-white/[0.025] p-3 sm:p-5">
        <div className="flex-1 overflow-y-auto pb-5">
          {isGenerating ? (
            <div className="flex min-h-[38vh] flex-col items-center justify-center text-center">
              ...
              <p className="mt-2 text-xs text-gray-500">{typeMeta.label} • {duration}s • {aspectRatio}</p>
            </div>
            ...
            <div className="mx-auto max-w-4xl">
              ...
              <button
                ...
                className="mt-3 inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.05] px-3 py-2 text-xs font-bold text-gray-200"
              >
            ...
            <div className="flex min-h-[38vh] flex-col items-center justify-center text-center">
              <span className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-pink-500/20 bg-pink-500/10">
                <Film className="h-5 w-5 text-pink-200" />
              </span>
              <p className="text-sm font-bold text-gray-300">Nouveau projet • {typeMeta.label}</p>
              <p className="mt-1 max-w-md text-xs leading-5 text-gray-600">{typeMeta.description}</p>
            </div>
          )}
        </div>
        ...
        {(startImage || referenceImages.length > 0) && (
          <div className="mb-2 flex gap-2 overflow-x-auto">
            <button
              type="button"
              onClick={() => { setStartImage(null); setReferenceImages([]); }}
              className="h-16 shrink-0 rounded-xl border border-white/10 px-3 text-[10px] font-bold text-gray-400"
            >
              Retirer tout
            </button>
            ...
            {referenceImages.map((src, index) => (
              <img key={index} src={src} alt={`Référence ${index + 1}`} className="h-16 w-16 shrink-0 rounded-xl object-cover" />
            ))}
          </div>
        )}
        ...
        <div className="rounded-3xl border border-white/10 bg-[#0b1426]/95 p-2 shadow-2xl">
          <input
            ...
            className="hidden"
            onChange={(event) => event.target.files && void loadImages(event.target.files)}
          />
          <div className="flex items-end gap-2">
            <button
              ...
              className="mb-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-white/[0.06] text-gray-300"
            >
            ...
            />
            <button type="button" onClick={() => setSettingsOpen(true)} className="mb-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-white/[0.06] text-gray-400">
            ...
            <button
              ...
              className="mb-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-tr from-purple-600 via-pink-600 to-blue-600 text-white disabled:opacity-40"
            >
          ...
          <aside className="flex h-full w-full max-w-md flex-col border-l border-white/10 bg-[#09111f]" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <section>
                ...
                <button
                  type="button"
                  onClick={() => setTypeListOpen((value) => !value)}
                  className="flex w-full items-center justify-between rounded-2xl border border-pink-400/25 bg-pink-500/[0.07] px-4 py-3 text-left"
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-black text-white">{typeMeta.label}</span>
                      <span className="rounded-full bg-pink-500/15 px-2 py-0.5 text-[9px] font-black text-pink-200">{typeMeta.badge}</span>
                    </div>
                    <p className="mt-1 text-[10px] text-gray-500">{typeMeta.description}</p>
                  </div>
                  <ChevronDown className={`h-4 w-4 text-gray-400 transition ${typeListOpen ? 'rotate-180' : ''}`} />
                </button>

                {typeListOpen && (
                  <div className="mt-2 max-h-[42vh] space-y-1 overflow-y-auto rounded-2xl border border-white/10 bg-[#07101f] p-2">
                    {TYPE_ORDER.map((key) => {
                      const meta = VIDEO_TYPE_ROUTING[key];
                      const active = key === videoType;
                      return (
                        <button
                          type="button"
                          key={key}
                          onClick={() => chooseType(key)}
                          className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left ${active ? 'bg-pink-500/10' : 'hover:bg-white/[0.04]'}`}
                        >
                          <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border ${active ? 'border-pink-400/35 bg-pink-500/15' : 'border-white/10 bg-white/[0.03]'}`}>
                            {active ? <Check className="h-3.5 w-3.5 text-pink-200" /> : <Film className="h-3.5 w-3.5 text-gray-500" />}
                          </span>
                          <div className="min-w-0 flex-1">
                            <p className="truncate text-xs font-black text-white">{meta.label}</p>
                            <p className="mt-0.5 line-clamp-1 text-[9px] text-gray-600">{meta.description}</p>
                          </div>
                        </button>
                      );
                    })}
              ...
              <section className="mt-6">
                ...
                {usesOmni ? (
                  ...
                  <>
                    <button
                      type="button"
                      onClick={() => setEngineListOpen((value) => !value)}
                      className="flex w-full items-center justify-between rounded-2xl border border-blue-400/20 bg-blue-500/[0.06] px-4 py-3 text-left"
                    >
                      <div>
                        ...
                        {recommendedEngines.map((engine, index) => {
                          ...
                          return (
                            <button
                              type="button"
                              key={engine.key}
                              onClick={() => chooseEngine(engine.key)}
                              className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left ${active ? 'bg-blue-500/10' : 'hover:bg-white/[0.04]'}`}
                            >
