...
const ENGINE_TO_MODEL: Partial<Record<VideoEngineKey, VideoModel>> = {
  'veo-lite': 'lite', 'veo-fast': 'fast', 'veo-pro': 'pro', omni: 'omni',
};
...
const SETTINGS_KEY = 'mungwele.video.smart-routing.v2';
...
export const VideoStudio: React.FC = () => {
  ...
  const chooseType = (next: VideoType) => {
    if (next === 'music_clip') {
      ...
      addNotification('info', 'Clips Vidéo', 'Pour un clip musical, MUNGWELE ouvre automatiquement le studio spécialisé.');
  ...
  const generate = async () => {
    ...
    const reason = `Vidéo ${typeMeta.label} — ${effectiveEngine.label} (${duration}s)`;
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100vh-8rem)] w-full max-w-5xl flex-col">
      ...
      {settingsOpen && <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm" onClick={() => setSettingsOpen(false)}><aside className="h-full w-full max-w-md overflow-y-auto border-l border-white/10 bg-[#09111f] p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between"><div><h2 className="text-lg font-black text-white">Réglages Vidéo</h2><p className="text-xs text-gray-500">Compact et adapté au type choisi.</p></div><button onClick={() => setSettingsOpen(false)} className="rounded-xl bg-white/[0.05] p-2"><X className="h-4 w-4" /></button></div>

        <section className="mt-6">
          <p className="mb-2 text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Type de vidéo</p>
          <button onClick={() => setTypeListOpen((v) => !v)} className="flex w-full items-center justify-between rounded-2xl border border-pink-400/25 bg-pink-500/[0.07] px-4 py-3 text-left"><div><div className="flex items-center gap-2"><span className="text-sm font-black text-white">{typeMeta.label}</span><span className="rounded-full bg-pink-500/15 px-2 py-0.5 text-[9px] font-black text-pink-200">{typeMeta.badge}</span></div><p className="mt-1 text-[10px] text-gray-500">{typeMeta.description}</p></div><ChevronDown className={`h-4 w-4 text-gray-400 transition ${typeListOpen ? 'rotate-180' : ''}`} /></button>
          {typeListOpen && <div className="mt-2 max-h-[48vh] space-y-1 overflow-y-auto rounded-2xl border border-white/10 bg-[#07101f] p-2">{TYPE_ORDER.map((key) => { const meta = VIDEO_TYPE_ROUTING[key]; const active = key === videoType; return <button key={key} onClick={() => chooseType(key)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left ${active ? 'bg-pink-500/10' : 'hover:bg-white/[0.04]'}`}><span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border ${active ? 'border-pink-400/35 bg-pink-500/15' : 'border-white/10 bg-white/[0.03]'}`}>{active ? <Check className="h-3.5 w-3.5 text-pink-200" /> : <Film className="h-3.5 w-3.5 text-gray-500" />}</span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate text-xs font-black text-white">{meta.label}</p><span className={`rounded-full px-2 py-0.5 text-[8px] font-black ${key === 'social' ? 'bg-emerald-500/15 text-emerald-300' : 'bg-white/[0.05] text-gray-500'}`}>{meta.badge}</span></div><p className="mt-0.5 line-clamp-1 text-[9px] text-gray-600">{meta.description}</p></div></button>; })}</div>}
        </section>

        <section className="mt-6"><p className="mb-2 text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Moteur disponible</p><div className="space-y-2">{connected.map((engine) => <button key={engine.key} onClick={() => { setEngineKey(engine.key); if (!engine.durations.includes(duration)) setDuration((engine.durations.includes(8) ? 8 : engine.durations[0]) as VideoDuration); }} className={`w-full rounded-xl border px-3 py-3 text-left ${engineKey === engine.key && !hasImages ? 'border-blue-400/35 bg-blue-500/10' : 'border-white/10 bg-white/[0.02]'}`}><div className="flex items-center justify-between"><span className="text-xs font-black text-white">{engine.label}</span><span className="text-[9px] text-emerald-300">Actif</span></div><p className="mt-1 text-[9px] text-gray-600">{engine.strengths.join(' • ')}</p></button>)}{hasImages && <div className="rounded-xl border border-purple-400/25 bg-purple-500/10 px-3 py-3 text-xs text-purple-200">Images détectées : Omni est utilisé automatiquement.</div>}</div></section>

        <section className="mt-6"><p className="mb-2 text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Durée disponible maintenant</p><div className="flex gap-2">{([4,6,8] as VideoDuration[]).filter((d) => effectiveEngine.durations.includes(d)).map((d) => <button key={d} onClick={() => setDuration(d)} className={`rounded-xl border px-4 py-2 text-xs font-black ${duration === d ? 'border-pink-400/35 bg-pink-500/10 text-pink-200' : 'border-white/10 text-gray-400'}`}>{d}s</button>)}</div></section>

        <section className="mt-6"><p className="mb-2 text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Format</p><div className="grid grid-cols-2 gap-2"><button onClick={() => setAspectRatio('9:16')} className={`rounded-xl border px-3 py-2 text-xs font-bold ${aspectRatio === '9:16' ? 'border-purple-400/35 bg-purple-500/10 text-purple-200' : 'border-white/10 text-gray-400'}`}>9:16 Mobile</button><button onClick={() => setAspectRatio('16:9')} className={`rounded-xl border px-3 py-2 text-xs font-bold ${aspectRatio === '16:9' ? 'border-purple-400/35 bg-purple-500/10 text-purple-200' : 'border-white/10 text-gray-400'}`}>16:9 Paysage</button></div></section>

        {planned.length > 0 && <section className="mt-6 rounded-2xl border border-amber-500/15 bg-amber-500/[0.04] p-3"><p className="text-[10px] font-black uppercase tracking-[0.12em] text-amber-300">Moteurs spécialisés à connecter</p><div className="mt-2 space-y-2">{planned.map((engine) => <div key={engine.key} className="flex items-center justify-between gap-3 text-[10px]"><div><p className="font-bold text-gray-300">{engine.label}</p><p className="text-gray-600">jusqu’à {engine.maxSeconds}s • {engine.strengths.slice(0,3).join(' • ')}</p></div><span className="shrink-0 rounded-full bg-amber-500/10 px-2 py-1 text-amber-300">À connecter</span></div>)}</div></section>}
      </aside></div>}
    </div>
  );
};
