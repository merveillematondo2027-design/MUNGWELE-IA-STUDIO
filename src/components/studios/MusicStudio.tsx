...
const SETTINGS_KEY = 'mungwele.music.settings.v2';
type MusicDuration = 30 | 60 | 120;
const MUSIC_COSTS: Record<MusicDuration, number> = { 30: 10, 60: 18, 120: 34 };
...
export const MusicStudio: React.FC = () => {
  ...
  const [durationSeconds, setDurationSeconds] = useState<MusicDuration>(30);
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (saved.durationSeconds === 30 || saved.durationSeconds === 60 || saved.durationSeconds === 120) setDurationSeconds(saved.durationSeconds);
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    try {
      ...
      if (settings?.genre) setGenre(String(settings.genre));
      if (settings?.mood) setMood(String(settings.mood));
      if (typeof settings?.isInstrumental === 'boolean') setInstrumental(settings.isInstrumental);
      if (settings?.durationSeconds === 30 || settings?.durationSeconds === 60 || settings?.durationSeconds === 120) setDurationSeconds(settings.durationSeconds);
  ...
  useEffect(() => {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({ genre, mood, instrumental, durationSeconds }));
  }, [genre, mood, instrumental, durationSeconds]);
  ...
  const generate = async () => {
    ...
    const reason = `Eleven Music ${durationSeconds}s — ${genre} / ${mood}`;
    ...
    try {
      ...
      triggerCelebration();
      addNotification('success', 'Musique prête', `Eleven Music v2 a généré votre morceau de ${durationSeconds} secondes.`);
    } catch (error: any) {
    ...
    <div className="mx-auto flex min-h-[calc(100vh-8rem)] w-full max-w-5xl flex-col">
      ...
      <div className="mb-4 flex items-center justify-between px-1">
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500">Eleven Music v2 • voix ou instrumental • MP3 haute qualité.</p>
        </div>
      ...
      <div className="flex flex-1 flex-col justify-end rounded-[28px] border border-white/10 bg-white/[0.025] p-3 sm:p-5">
        <div className="flex flex-1 items-center justify-center pb-6">
          {isGenerating ? (
            <div className="text-center">
              ...
              <p className="text-sm font-black text-white">Production du morceau…</p>
              <p className="mt-2 text-xs text-gray-600">Eleven Music v2 • {durationSeconds}s • {instrumental ? 'instrumental' : 'avec voix'}</p>
            </div>
          ) : latest?.resultUrl ? (
            ...
            <div className="text-center">
              ...
              <p className="text-sm font-bold text-gray-300">Votre prochain morceau commence ici.</p>
              <p className="mt-2 max-w-md text-xs leading-5 text-gray-600">Décrivez genre, ambiance, instruments, tempo, voix et intention. Vous pouvez aussi laisser Gemini préparer vos paroles.</p>
            </div>
          )}
        </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-[#0b1426]/95 p-2 shadow-2xl">
          <div className="flex items-end gap-2">
            <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); generate(); } }} rows={2} disabled={isGenerating} placeholder="Ex. Afrobeats congolais joyeux, guitare mélodique, basse chaude, voix masculine en français…" className="max-h-40 min-h-[44px] flex-1 resize-none bg-transparent px-3 py-3 text-sm text-white outline-none placeholder:text-gray-600 disabled:opacity-60" />
          ...
          <div className="flex items-center justify-between px-3 pb-1 pt-2 text-[10px] text-gray-600"><span>{genre} • {mood} • {durationSeconds}s • {instrumental ? 'Instrumental' : 'Avec voix'}</span><span>{creditCost} crédits</span></div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm" onClick={() => setSettingsOpen(false)}>
          <aside className="h-full w-full max-w-md overflow-y-auto border-l border-white/10 bg-[#09111f] p-5" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <div><h2 className="text-lg font-black text-white">Réglages Musique</h2><p className="text-xs text-gray-500">Conservés automatiquement.</p></div>
              <button onClick={() => setSettingsOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/[0.05]"><X className="h-4 w-4" /></button>
            </div>
            <div className="mt-8 space-y-6">
              <label className="block"><span className="mb-2 block text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Genre</span><input value={genre} onChange={(e) => setGenre(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none" /></label>
              <label className="block"><span className="mb-2 block text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Ambiance</span><input value={mood} onChange={(e) => setMood(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white outline-none" /></label>
              <section>
                <span className="mb-2 block text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Durée</span>
                <div className="grid grid-cols-3 gap-2">
                  {([30, 60, 120] as MusicDuration[]).map((value) => <button key={value} onClick={() => setDurationSeconds(value)} className={`rounded-2xl border py-3 text-xs font-black ${durationSeconds===value?'border-amber-400/50 bg-amber-400/10 text-amber-200':'border-white/10 text-gray-400'}`}>{value === 120 ? '2 min' : `${value}s`}<span className="mt-1 block text-[9px] font-medium opacity-70">{MUSIC_COSTS[value]} crédits</span></button>)}
                </div>
              </section>
              <section>
                <span className="mb-2 block text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Type</span>
                <div className="grid grid-cols-2 gap-2"><button onClick={() => setInstrumental(false)} className={`rounded-2xl border py-3 text-xs font-black ${!instrumental?'border-blue-500 bg-blue-500/10 text-white':'border-white/10 text-gray-400'}`}>Avec voix</button><button onClick={() => setInstrumental(true)} className={`rounded-2xl border py-3 text-xs font-black ${instrumental?'border-blue-500 bg-blue-500/10 text-white':'border-white/10 text-gray-400'}`}>Instrumental</button></div>
              </section>
              {!instrumental && (
                <section>
                  <div className="mb-2 flex items-center justify-between"><span className="text-[11px] font-black uppercase tracking-[0.12em] text-gray-500">Paroles</span><button onClick={generateLyrics} disabled={isGeneratingLyrics} className="text-[11px] font-bold text-blue-300">{isGeneratingLyrics ? 'Génération…' : 'Générer par IA'}</button></div>
                  <textarea value={lyrics} onChange={(e) => setLyrics(e.target.value)} rows={9} placeholder="Laissez vide pour laisser Eleven Music créer les paroles, ou générez/écrivez les vôtres." className="w-full rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-xs leading-5 text-white outline-none placeholder:text-gray-600" />
                </section>
              )}
              <div className="rounded-2xl border border-emerald-500/15 bg-emerald-500/[0.05] p-4 text-[11px] leading-5 text-gray-400"><strong className="text-emerald-300">Protection MUNGWELE :</strong> le coût varie avec la durée. En cas d’échec fournisseur, les crédits de l’utilisateur sont remboursés automatiquement.</div>
            </div>
