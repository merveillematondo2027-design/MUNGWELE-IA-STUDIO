      ...
      {section === 'credits' && (
        <>
          <section className="space-y-4">
            <div><h2 className="text-2xl font-black text-white">Acheter des crédits</h2><p className="mt-1 text-xs text-gray-400">Saisie manuelle, QR et NFC Market-Cash restent disponibles au paiement.</p></div>
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-5 md:grid-cols-3">
              {plans.map((plan) => {
                ...
                return <article key={plan.id} className={`relative flex flex-col rounded-3xl border p-5 ${plan.popular ? 'border-purple-500 bg-purple-950/25' : 'border-white/10 bg-white/[0.03]'}`}>{plan.popular && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 px-3 py-1 text-[10px] font-black text-white">POPULAIRE</span>}<h3 className="text-xl font-black text-white">{plan.name}</h3><p className="mt-2 text-4xl font-black text-white">${discountedMonthly.toFixed(discountedMonthly % 1 ? 2 : 0)}<span className="text-xs font-medium text-gray-500"> / mois</span></p>{billingCycle === 'yearly' && plan.id !== 'free' && <p className="mt-1 text-[11px] font-bold text-cyan-300">Facturé ${amountDue.toFixed(2)} pour 12 mois</p>}<div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.04] p-3"><div className="flex justify-between text-xs"><span className="text-gray-400">Crédits mensuels</span><strong className="text-amber-300">{plan.creditsMonthly}</strong></div></div><ul className="mt-4 flex-1 space-y-2">{plan.features.map((feature, index) => <li key={index} className="flex gap-2 text-xs text-gray-300"><Check className="h-4 w-4 shrink-0 text-purple-400" />{feature}</li>)}</ul><button disabled={current || plan.id === 'free'} onClick={() => openPayment({ kind: 'subscription', label: `Abonnement ${plan.name}`, amountUsd: amountDue, metadata: { planId: plan.id, billingCycle } })} className={`mt-5 w-full rounded-xl py-3 text-xs font-black ${current || plan.id === 'free' ? 'border border-white/10 bg-white/[0.03] text-gray-500' : 'bg-gradient-to-r from-purple-600 to-pink-600 text-white'}`}>{current ? 'Formule actuelle' : plan.id === 'free' ? 'Sans abonnement' : `Payer ${plan.name} avec Market-Cash`}</button></article>;
              })}
            </div>
          </section>
          <section className="rounded-3xl border border-white/10 bg-white/[0.025] p-5"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-300" /><div><h3 className="text-sm font-black text-white">Paiement carte protégé</h3><p className="mt-1 text-xs leading-5 text-gray-400">Le CVV reste saisi manuellement pour les paiements ordinaires et n’est jamais conservé par MUNGWELE. Si vous autorisez le renouvellement automatique, Market-Cash fournit à MUNGWELE une autorisation de paiement révocable, sans partager le CVV.</p></div></div><div className="mt-4 overflow-hidden rounded-2xl border border-white/10">{VIDEO_ROWS.map(([name, cost, duration]) => <div key={name} className="grid grid-cols-[1.2fr_1fr_auto] gap-3 border-b border-white/5 px-4 py-3 text-xs"><span className="font-bold text-white">{name}</span><span className="text-amber-300">{cost}</span><span className="text-gray-500">{duration}</span></div>)}</div></section>
