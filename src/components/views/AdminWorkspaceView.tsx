...
import { ProviderWalletPanel } from '../admin/ProviderWalletPanel';
...
const sectionMeta = [
  ...
  { key: 'credits' as const, title: 'Gestion des crédits', subtitle: 'Magasin API, soldes clients et mouvements', icon: Coins },
...
export const AdminWorkspaceView: React.FC<{ section: AdminSection }> = ({ section }) => {
  ...
  if (section === 'credits') return <div className="mx-auto w-full max-w-6xl pb-16"><PageHeader title="Gestion des crédits" subtitle="Magasin API fournisseur, soldes utilisateurs et historique des mouvements." /><ProviderWalletPanel/><h2 className="mb-3 text-sm font-black text-white">Soldes utilisateurs</h2><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{users.map((u)=><button key={u.id} onClick={()=>{setSelectedUserId(u.id);openSection('users')}} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left"><p className="font-bold text-white">{u.name||u.email}</p><p className="mt-2 text-2xl font-black text-amber-300">{u.credits||0}</p><p className="text-[10px] text-gray-600">crédits disponibles</p></button>)}</div><h2 className="mb-3 mt-7 text-sm font-black text-white">Mouvements clients</h2><div className="space-y-2">{transactions.slice().sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))).map((t)=><div key={t.id} className="flex justify-between rounded-xl border border-white/10 bg-white/[0.02] p-3 text-xs"><div><p className="font-bold text-gray-200">{t.description}</p><p className="text-gray-600">{fmt(t.createdAt)}</p></div><span className={t.amount>=0?'text-emerald-300':'text-rose-300'}>{t.amount>=0?'+':''}{t.amount}</span></div>)}</div></div>;
