...
import { ArrowRight, Clapperboard, Coins, Film, Image as ImageIcon, Music as MusicIcon, Sparkles } from 'lucide-react';
...
export const HomeView: React.FC = () => {
  ...
  const balance = user.role === 'admin' ? '∞' : String(Math.max(0, Number(user.credits || 0)));
  ...
  return (
    <div className="mx-auto w-full max-w-6xl pb-12 pt-2 sm:pt-6">
      <div className="mx-auto mb-5 max-w-2xl text-center sm:mb-7">
        ...
        {user.id && <button onClick={()=>setActiveTab('subscription')} className="mx-auto mt-3 inline-flex items-center gap-2 rounded-full border border-amber-400/20 bg-amber-400/[0.06] px-3 py-1.5 text-[11px] font-black text-amber-200"><Coins className="h-3.5 w-3.5"/><span>{balance} crédits</span></button>}
      </div>
