    ...
    <div className="mx-auto w-full max-w-6xl pb-16">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0"><div className="flex items-center gap-2"><Icon className={`h-5 w-5 ${meta.accent}`} /><h1 className="truncate text-xl font-black text-white sm:text-2xl">{meta.title}</h1></div><p className="mt-1 text-xs text-gray-500">Ouvrez un projet existant ou démarrez une nouvelle création.</p></div>
        </div>
          ...
          {projects.map((project) => (
            <button key={project.id} onClick={() => resume(project)} className="group overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] text-left transition hover:border-white/20 hover:bg-white/[0.05]">
              <div className="relative aspect-video overflow-hidden bg-black/30">
                {type === 'music' ? <div className="flex h-full items-center justify-center"><Music2 className="h-10 w-10 text-blue-300/70" /></div> : type === 'clips' && !project.thumbnailUrl && !project.resultUrl ? <div className="flex h-full items-center justify-center"><Clapperboard className="h-10 w-10 text-fuchsia-300/70" /></div> : project.thumbnailUrl || project.resultUrl ? <img src={project.thumbnailUrl || project.resultUrl} alt={project.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" /> : <div className="flex h-full items-center justify-center"><Film className="h-10 w-10 text-gray-600" /></div>}
              ...
              <div className="p-3.5"><div className="flex items-center gap-2"><h2 className="line-clamp-1 flex-1 text-sm font-black text-white">{project.title || 'Projet sans titre'}</h2>{project.status === 'draft' && <span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-[9px] font-bold text-amber-300">Brouillon</span>}{project.isPublic && <span className="rounded-full bg-cyan-500/10 px-2 py-0.5 text-[9px] font-bold text-cyan-300">Public</span>}</div><p className="mt-1 line-clamp-2 text-[11px] leading-4 text-gray-500">{project.prompt}</p><div className="mt-3 flex items-center justify-between text-[10px] text-gray-600"><span>{new Date(project.updatedAt || project.createdAt).toLocaleDateString('fr-FR')}</span><span className="font-bold text-gray-300">Continuer</span></div></div>
            </button>
          ))}
        </div>
      )}
