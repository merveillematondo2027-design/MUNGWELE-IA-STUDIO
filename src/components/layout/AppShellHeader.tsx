...
import { ArrowLeft, ChevronDown, Menu, X, UserRound, LogOut, Images, CreditCard, CircleHelp, Settings, Shield, Globe2, Home, Image as ImageIcon, Film, Music2 } from 'lucide-react';
...
export const AppShellHeader: React.FC<{ studioMode?: boolean }> = ({ studioMode = false }) => {
  ...
  const goStudioProjects = (studio: 'image' | 'video' | 'music') => {
    ...
    <>
        ...
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative z-50">
              ...
              {profileOpen && (
                <>
                  <button aria-label="Fermer le menu profil" onClick={() => setProfileOpen(false)} className="fixed inset-0 z-[-1] cursor-default bg-transparent" />
                  <div className="absolute right-0 mt-2 w-64 overflow-hidden rounded-2xl border border-white/10 bg-[#0b1324] p-2 shadow-2xl">
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm" onClick={() => setMenuOpen(false)}>
          <aside className="h-full w-full max-w-sm overflow-y-auto border-l border-white/10 bg-[#09111f] p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-1">
              <button onClick={goHome} className="flex w-full items-center gap-3 rounded-2xl bg-white/[0.035] px-4 py-3 text-sm font-bold text-white hover:bg-white/[0.07]"><Home className="h-4 w-4 text-white" /> Accueil</button>
              <div className="my-3 h-px bg-white/10" />
              <button onClick={() => goStudioProjects('image')} className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm text-gray-200 hover:bg-white/[0.06]"><ImageIcon className="h-4 w-4 text-purple-300" /> Image</button>
              <button onClick={() => goStudioProjects('video')} className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm text-gray-200 hover:bg-white/[0.06]"><Film className="h-4 w-4 text-pink-300" /> Vidéo</button>
              <button onClick={() => goStudioProjects('music')} className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm text-gray-200 hover:bg-white/[0.06]"><Music2 className="h-4 w-4 text-blue-300" /> Musique</button>
