...
export const AppShellHeader: React.FC<{ studioMode?: boolean }> = ({ studioMode = false }) => {
  const { user, setActiveTab, setActiveStudio, setAuthMode, setIsAuthModalOpen } = useApp();
  ...
  const openAuth = (mode: 'login' | 'register') => { setAuthMode(mode); setIsAuthModalOpen(true); closeMenus(); };
  ...
  const logout = async () => { await auth.signOut(); setProfileOpen(false); goHome(); };

  useEffect(() => {
    if (!signedIn) { setUnreadNotifications(0); return; }
  ...
  useEffect(() => {
    if (!signedIn) { setUnreadMessages(0); return; }
  ...
  return <>
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#070d1b]/90 backdrop-blur-2xl">
      <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8">
        <div className="flex min-w-0 items-center gap-2">{signedIn&&studioMode&&<button onClick={goHome} title="Retour" className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-gray-300"><ArrowLeft className="h-4 w-4"/></button>}<button onClick={goHome}><img src={officialLogo} alt="MUNGWELE AI STUDIO" className="h-11 w-[92px] object-contain object-left sm:w-[142px]"/></button></div>

        {!signedIn ? (
          <div className="flex items-center gap-2">
            <button onClick={()=>openAuth('login')} className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2.5 text-xs font-black text-white sm:px-4">Connexion</button>
            <button onClick={()=>openAuth('register')} className="rounded-xl bg-gradient-to-r from-purple-600 via-pink-600 to-blue-600 px-3 py-2.5 text-xs font-black text-white shadow-lg sm:px-4">S'inscrire</button>
          </div>
        ) : (
