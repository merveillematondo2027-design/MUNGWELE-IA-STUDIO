...
import { ArrowLeft, ChevronDown, Menu, X, UserRound, LogOut, Images, CreditCard, CircleHelp, Settings, Shield, Coins } from 'lucide-react';
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#070d1b]/85 backdrop-blur-2xl">
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-2 sm:gap-3">
            {studioMode && (
              <button onClick={goHome} className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-gray-300 hover:bg-white/[0.08] hover:text-white" title="Retour à l'accueil">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setMenuOpen(true)} className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 text-xs font-bold text-gray-200 hover:bg-white/[0.08]">
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-60 overflow-hidden rounded-2xl border border-white/10 bg-[#0b1324] p-2 shadow-2xl">
                  <div className="px-3 py-2 border-b border-white/10 mb-1">
                    <p className="truncate text-xs font-bold text-white">{user.name || 'Compte MUNGWELE'}</p>
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm" onClick={() => setMenuOpen(false)}>
          <aside className="h-full w-full max-w-sm border-l border-white/10 bg-[#09111f] p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="mb-6 flex items-center justify-between">
              <div><p className="text-sm font-black text-white">Menu</p><p className="text-[11px] text-gray-500">Tout le reste, sans encombrer les studios.</p></div>
              <button onClick={() => setMenuOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/[0.05]"><X className="h-4 w-4" /></button>
            </div>
            ...
            <div className="mt-auto pt-8 text-[10px] text-gray-600">MH APIS — centre d’API Mungwele Holding, prévu pour une phase ultérieure.</div>
          </aside>
        </div>
      )}
    </>
