      ...
      <section className="mt-7 overflow-hidden rounded-[28px] border border-red-500/20 bg-gradient-to-br from-red-950/25 via-white/[0.025] to-emerald-950/15 p-5 sm:p-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-black text-white">Paiement Mobile Money RDC</h3><span className="rounded-full border border-emerald-500/20 bg-emerald-950/20 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-emerald-300">M-Pesa C2B Production</span></div>
              <p className="mt-2 max-w-2xl text-xs leading-5 text-gray-400">Paiement réel M-Pesa pour les crédits et abonnements MUNGWELE. Le connecteur est préparé pour l’environnement Live ; il reste désactivé tant que les identifiants de production Vodacom/M-Pesa ne sont pas renseignés.</p>
              <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold"><span className="rounded-full bg-red-500/10 px-2.5 py-1 text-red-200">M-Pesa · Production</span><span className="rounded-full bg-white/[0.05] px-2.5 py-1 text-gray-400">Airtel Money · préparé</span><span className="rounded-full bg-white/[0.05] px-2.5 py-1 text-gray-400">Orange Money · préparé</span></div>
            </div>
          </div>
      ...
      {chooserOpen && (
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/80 p-3 backdrop-blur-sm">
          <div className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-white/10 bg-[#0d1420] p-5 shadow-2xl sm:p-6">
            <div className="flex items-start justify-between gap-4"><div><div className="inline-flex items-center gap-2 rounded-full border border-red-500/20 bg-red-950/20 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-red-200"><Zap className="h-3.5 w-3.5" /> Choisir l’achat</div><h3 className="mt-3 text-2xl font-black text-white">Que voulez-vous payer ?</h3><p className="mt-1 text-xs text-gray-400">Le prix est toujours recalculé côté serveur avant l’envoi à M-Pesa.</p></div><button onClick={() => setChooserOpen(false)} className="rounded-full border border-white/10 bg-white/[0.04] p-2.5 text-gray-400 hover:text-white"><X className="h-5 w-5" /></button></div>
