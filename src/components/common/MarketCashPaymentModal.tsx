...
import { Camera, CheckCircle2, CreditCard, Loader2, Nfc, ScanLine, ShieldCheck, X } from 'lucide-react';
import {
  normalizeCardNumber,
  normalizeExpiry,
...
export const MarketCashPaymentModal: React.FC<MarketCashPaymentModalProps> = ({ target, userId, userEmail, onClose, onSuccess }) => {
  ...
  const scanQr = async () => {
    ...
    try {
      ...
      if (!Detector) throw new Error("Le scan QR n'est pas disponible dans ce navigateur. Utilisez la saisie manuelle.");
      ...
      throw new Error('QR non détecté. Réessayez ou saisissez la carte manuellement.');
    } catch (e: any) {
  ...
  const readNfc = async () => {
    ...
    try {
      ...
      if (!Reader) throw new Error("Le NFC Web n'est pas disponible sur cet appareil ou navigateur. Utilisez le QR ou la saisie manuelle.");
      const reader = new Reader();
      await reader.scan();
      const raw = await new Promise<string>((resolve, reject) => {
        const timer = window.setTimeout(() => reject(new Error('Aucune carte NFC détectée. Réessayez en approchant la carte du téléphone.')), 20000);
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 p-3 backdrop-blur-sm">
      <div className="max-h-[94vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-white/10 bg-[#0d1420] shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-white/10 p-5 sm:p-6">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-emerald-300">Paiement sécurisé Market-Cash</p>
            <h3 className="mt-2 text-xl font-black text-white">{target.label}</h3>
            <p className="mt-1 text-sm font-bold text-amber-300">${target.amountUsd.toFixed(2)} USD</p>
          </div>
          <button onClick={onClose} className="rounded-xl bg-white/5 p-2 text-gray-300 hover:bg-white/10" aria-label="Fermer"><X className="h-5 w-5" /></button>
        </div>
        ...
        <form onSubmit={submit} className="space-y-5 p-5 sm:p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <button type="button" onClick={() => { stopCamera(); setCaptureMethod('manual'); }} className={`rounded-2xl border p-4 text-left ${captureMethod==='manual'?'border-purple-500 bg-purple-950/30':'border-white/10 bg-white/[0.02]'}`}><CreditCard className="h-5 w-5 text-purple-300"/><p className="mt-2 text-xs font-black text-white">Saisie manuelle</p></button>
            <button type="button" onClick={scanQr} disabled={scanBusy} className={`rounded-2xl border p-4 text-left ${captureMethod==='qr'?'border-cyan-500 bg-cyan-950/30':'border-white/10 bg-white/[0.02]'}`}><ScanLine className="h-5 w-5 text-cyan-300"/><p className="mt-2 text-xs font-black text-white">Scanner le QR</p></button>
            <button type="button" onClick={readNfc} disabled={scanBusy} className={`rounded-2xl border p-4 text-left ${captureMethod==='nfc'?'border-emerald-500 bg-emerald-950/30':'border-white/10 bg-white/[0.02]'}`}><Nfc className="h-5 w-5 text-emerald-300"/><p className="mt-2 text-xs font-black text-white">Approcher la carte NFC</p></button>
          </div>

          <video ref={videoRef} className={`w-full rounded-2xl border border-white/10 bg-black ${scanBusy ? 'block' : 'hidden'}`} muted playsInline />
          {scanBusy && <div className="flex items-center gap-2 text-xs text-cyan-300"><Camera className="h-4 w-4"/><span>Lecture en cours…</span></div>}
          ...
          <p className="text-center text-[10px] leading-4 text-gray-500">Les données carte sont envoyées uniquement au flux de paiement. Le CVV n'est pas conservé dans l'application.</p>
        </form>
      </div>
    </div>
  );
