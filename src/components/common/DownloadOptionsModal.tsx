...
export const DownloadOptionsModal: React.FC<{
  ...
  const hasPaidSubscription = user.role === 'admin' || user.plan === 'creator' || user.plan === 'pro' || user.plan === 'studio';
  const videoWatermarkApplies = isVideo && (community || !hasPaidSubscription);
  ...
  const start = async (optionId: string) => {
    ...
    try {
      ...
      addNotification(
        ...
        isVideo
          ? community
            ? `Vidéo MP4 préparée avec la signature MUNGWELE • ${ownerName || 'Créateur'}.`
            : videoWatermarkApplies
              ? 'Vidéo MP4 préparée avec le filigrane MUNGWELE. Un abonnement actif permet le téléchargement sans filigrane.'
              : 'Vidéo MP4 préparée sans filigrane MUNGWELE grâce à votre abonnement actif.'
          : community
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/75 p-4 backdrop-blur-md" onClick={onClose}>
    <div className="w-full max-w-lg rounded-3xl border border-white/10 bg-[#0a1324] p-5 shadow-2xl" onClick={(e)=>e.stopPropagation()}>
      ...
      {isVideo && community
        ? <div className="mt-4 rounded-2xl border border-fuchsia-500/20 bg-fuchsia-500/[0.05] p-3 text-xs leading-5 text-gray-300">Téléchargement communauté : la vidéo conserve la <strong className="text-fuchsia-200">signature MUNGWELE + identité du créateur</strong>.</div>
        : isVideo && videoWatermarkApplies
          ? <div className="mt-4 rounded-2xl border border-amber-500/20 bg-amber-500/[0.05] p-3 text-xs leading-5 text-gray-300">Formule gratuite : le téléchargement 480p porte le <strong className="text-amber-200">filigrane MUNGWELE</strong>. Creator, Pro et Studio téléchargent leurs créations personnelles sans filigrane.</div>
          : isVideo
            ? <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.05] p-3 text-xs leading-5 text-gray-300">Abonnement actif : vos téléchargements personnels sont préparés <strong className="text-emerald-200">sans filigrane MUNGWELE</strong>.</div>
