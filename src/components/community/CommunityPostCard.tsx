...
import { BadgeCheck, Download, Heart, MessageCircle, MessageSquareText, Send, Share2, UserCheck, UserPlus } from 'lucide-react';
...
export const CommunityPostCard: React.FC<{ post: CommunityPost }> = ({ post }) => {
  ...
  const onFollow = async () => { await toggleFollow(user.id, targetId, user.name, following); if (!following && !post.isOfficial) await notifyUser(post.userId, user, 'follow', `${user.name} s’est abonné à votre compte.`); };
  const onShare = async () => { await sharePost(post); addNotification('success', 'Partage prêt', 'Le partage interne/externe est disponible.'); };
  const onMessage = async () => { if (post.isOfficial) { addNotification('info', 'Compte officiel', 'La messagerie du compte officiel sera gérée par le support MUNGWELE.'); return; } await sendMessage(user, post.userId, post.authorName, `Bonjour ${post.authorName}, je vous écris à propos de votre publication « ${post.title} ».`); addNotification('success', 'Conversation créée', 'Retrouvez-la dans Messages.'); };
  ...
  return <article className="overflow-hidden rounded-3xl border border-white/10 bg-[#0a1324]/95 shadow-xl">
    ...
    <div className="flex items-center justify-between border-b border-white/10 px-4 py-2 text-[10px] text-gray-500"><span>{likes.length} J’aime</span><div className="flex items-center gap-3"><button onClick={() => setCommentsOpen((v) => !v)}>{comments.length} commentaire{comments.length > 1 ? 's' : ''}</button>{post.allowCommunityDownload&&<button onClick={()=>setDownloadOpen(true)} className="inline-flex items-center gap-1 text-cyan-300"><Download className="h-3 w-3"/> Télécharger</button>}</div></div>
