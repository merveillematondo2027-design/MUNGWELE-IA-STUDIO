...
export const CommunityPostCard: React.FC<{ post: CommunityPost }> = ({ post }) => {
  ...
  const onFollow = async () => { try { await toggleFollow(user.id, post.userId, user.name, following); if (!following) await notifyUser(post.userId, user, 'follow', `${user.name} s’est abonné à votre compte.`); } catch(error:any){ addNotification('error','Abonnement impossible',error?.message || 'Vérifiez vos permissions.'); } };
  ...
  const onMessage = async () => { if (official) { addNotification('info', 'Compte officiel', 'La messagerie du compte officiel sera reliée au support MUNGWELE dans l’espace support.'); return; } try { await sendMessage(user, post.userId, visibleName, `Bonjour ${visibleName}, je vous écris à propos de votre publication « ${post.title} ».`); addNotification('success', 'Conversation créée', 'Retrouvez-la dans Messages.'); } catch(error:any){ addNotification('error','Message impossible',error?.message || 'Vérifiez vos permissions.'); } };
