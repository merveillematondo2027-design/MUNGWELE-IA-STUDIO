...
export const CommunityPostCard: React.FC<{ post: CommunityPost }> = ({ post }) => {
  ...
  const downloadItem: GenerationRecord = {
    ...
    createdAt:post.createdAt, updatedAt:post.createdAt, isPublic:true, authorName:post.authorName,
  ...
  const onLike = async () => { try { await toggleLike(post.id, user, liked); if (!liked) await notifyUser(post.userId, user, 'like', `${user.name} aime votre publication.`, post.id); } catch(error:any){ addNotification('error','J’aime impossible',error?.message || 'Vérifiez vos permissions.'); } };
  const onComment = async () => { const value = comment.trim(); if (!value) return; try { await addComment(post.id, user, value); await notifyUser(post.userId, user, 'comment', `${user.name} a commenté votre publication.`, post.id); setComment(''); setCommentsOpen(true); } catch(error:any){ addNotification('error','Commentaire impossible',error?.message || 'Vérifiez vos permissions.'); } };
  const onFollow = async () => { try { await toggleFollow(user.id, targetId, user.name, following); if (!following && !post.isOfficial) await notifyUser(post.userId, user, 'follow', `${user.name} s’est abonné à votre compte.`); } catch(error:any){ addNotification('error','Abonnement impossible',error?.message || 'Vérifiez vos permissions.'); } };
  const onShare = async () => { await sharePost(post); addNotification('success', 'Partage prêt', 'Le partage interne/externe est disponible.'); };
  const onMessage = async () => { if (post.isOfficial) { addNotification('info', 'Compte officiel', 'La messagerie du compte officiel sera reliée au support MUNGWELE dans l’espace support.'); return; } try { await sendMessage(user, post.userId, post.authorName, `Bonjour ${post.authorName}, je vous écris à propos de votre publication « ${post.title} ».`); addNotification('success', 'Conversation créée', 'Retrouvez-la dans Messages.'); } catch(error:any){ addNotification('error','Message impossible',error?.message || 'Vérifiez vos permissions.'); } };
  ...
  const onReport = async () => { try { await reportPost(post,user,'Contenu signalé depuis la communauté'); setActionsOpen(false); addNotification('success','Signalement envoyé','Merci. Le contenu a été transmis à la modération MUNGWELE.'); } catch(error:any){ addNotification('error','Signalement impossible',error?.message || 'Vérifiez vos permissions.'); } };
  ...
  return <article className="relative overflow-hidden rounded-3xl border border-white/10 bg-[#0a1324]/95 shadow-xl">
    ...
    <DownloadOptionsModal item={downloadOpen?downloadItem:null} onClose={()=>setDownloadOpen(false)} community ownerName={post.authorName}/>
  </article>;
};
