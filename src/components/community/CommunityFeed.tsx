...
export const CommunityFeed: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  ...
  return <section className={compact ? 'mt-5' : ''}>
    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><Globe2 className="h-5 w-5 text-cyan-300"/><h2 className="text-lg font-black text-white sm:text-xl">Communauté MUNGWELE</h2></div><p className="mt-1 text-xs text-gray-500">Publications, abonnements, J’aime, commentaires, téléchargements autorisés et partages en temps réel.</p></div><div className="flex items-center gap-2 text-[10px] text-gray-500"><SlidersHorizontal className="h-3.5 w-3.5"/> Rechercher et filtrer</div></div>
